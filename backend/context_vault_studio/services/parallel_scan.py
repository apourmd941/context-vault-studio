...
def build_parallel_scan_profile(config: dict, *, base_dir: Path | None = None, max_workers: int = 4) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=min(max_workers, max(len(sources), 1))) as executor:
